...
veryOldInventors = inventors.filter((born) => {
    if(born.year >= 1500 && born.year < 1600){
        return born
    }
})
...
inventorNames = inventors.map((names) => {
    return `{first: ${names.first}, last: ${names.last}}`
})
...
sortedByBirthYear = inventors.sort((born1, born2) => born1.year > born2.year ? 1 : -1) 
...
isEveryone19OrOlder = devs.every((age) =>{
    return 2024 - age.year >= 18
})
...
idx = comments.findIndex((indx) => {
    return indx.id === 123523
})
...
totalYearsLived = inventors.reduce((acc, age) => {
    return acc += age.passed - age.year
}, 0)
